...
- **드래그 앤 드롭 업로드** — 파일을 드롭존에 끌어다 놓거나 클릭하여 선택
- **단계별 처리 상태 표시** — 파일 확인 중 → 음성 추출/전처리 중 → 서버 처리 중
- **처리 시간 안내 및 단계별 오류 메시지** — 형식 오류·추출 실패·서버 오류를 각각 구분하여 표시
- **ElevenLabs Scribe v1 자동 전사 (STT)** — 원본 언어 자동 감지 후 결과에 표시
- **DeepL 번역** — 한국어·일본어·중국어(간체)·스페인어·프랑스어·독일어·포르투갈어·이탈리아어·러시아어·영어 (10개 언어)
- **ElevenLabs `eleven_multilingual_v2` 음성 합성 (TTS)**
- **결과 표시** — 원문 전사(감지 언어 표시)·번역 텍스트·더빙 오디오 재생·MP3 다운로드·전사/번역 TXT 다운로드
- **파일 교체 시 결과 자동 초기화** — 새 파일 선택 즉시 이전 결과 제거
- **다시 더빙하기** — 결과 화면 하단 버튼으로 폼 초기화
...
- **기능 확장 — 최소 수정 원칙:** `transcribe()` 반환값에 `languageCode` 필드를 추가하고, API 응답과 클라이언트 타입을 함께 확장하는 방식으로 원본 언어 감지 표시를 구현했습니다. 드래그 앤 드롭은 기존 `handleFileChange` 로직을 `selectFile()` 헬퍼로 분리한 뒤 드롭 핸들러에서 재사용하는 방식으로 기존 코드를 최대한 유지했습니다.
...
| 5 | 결과 확인 | 원문 전사(감지 언어 표시)·번역 텍스트·오디오 재생·MP3 다운로드·TXT 다운로드 모두 정상 |
| 6 | 비허가 계정 로그인 | `/blocked` 페이지 이동, 세션 미생성 |
| 7 | 로그아웃 | `/login` 리다이렉트, 이후 `/dub` 직접 접근 시 `/login` 리다이렉트 |
| 8 | **드래그 앤 드롭** | 파일을 드롭존에 드래그 시 파란 테두리·"여기에 놓으세요" 표시 → 드롭 시 파일 선택됨 |
| 9 | **파일 교체** | 더빙 완료 후 새 파일 선택 시 이전 결과 즉시 사라짐 |
| 10 | **다시 더빙하기** | 결과 하단 버튼 클릭 시 폼 초기 상태로 리셋 |

### 모바일

| # | 환경 | 확인 항목 |
|---|---|---|
| 11 | **Android Chrome** | MP4 영상 업로드 → 오디오 추출 및 더빙 완료 |
| 12 | **iPhone Safari** | MP4 영상(카메라 촬영본) 업로드 → 오디오 추출 및 더빙 완료 |
| 13 | **iPhone Safari** | MOV 파일 업로드 시 오류 메시지 표시 (조용히 실패하지 않음) |

### 오류 처리 및 유효성 검사

| # | 시나리오 | 기대 동작 |
|---|---|---|
| 14 | 지원되지 않는 영상 코덱(MOV 등) 업로드 | 추출 실패 오류 메시지 (코덱 안내 포함), 서버 미전송 |
| 15 | `.docx` 등 미지원 파일 형식 제출 | 즉시 "지원하지 않는 파일 형식" 오류 표시, 서버 미전송 |
| 16 | **500 MB 초과** 파일 제출 | 즉시 파일 크기 초과 오류 표시 (MB 수치 포함), 서버 미전송 |
| 17 | 환경 변수 미설정 상태로 더빙 시도 | 서버에서 명확한 오류 메시지 반환 |
| 18 | 더빙 대기 상태에서 처리 시간 안내 확인 | 제출 버튼 하단에 "보통 15~45초 정도 소요될 수 있어요." 표시 |
